...
describe("MultiCall", function () {
    ...
    beforeEach(async () => {
        ...
        const transaction = await token0.connect(signer).transfer(owner.address, amount)
    ...
    describe("Multiswap Arbitrage", () => {
        ...
        it('Updates the contract balance', async () => {
            expect(Number(await token0.balanceOf(multiCall.address))).to.be.greaterThan(0)
        })
        ...
        it('Allows withdraw to owner', async () => {
            ...
            expect(Number(await token0.balanceOf(owner.address))).to.be.greaterThan(0)
